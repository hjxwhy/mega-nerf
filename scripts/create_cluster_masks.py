        ...
        for i in main_tqdm(np.arange(rank, len(metadata_paths), world_size)):
            ...
            for j in range(0, rays.shape[0], hparams.ray_chunk_size):
                ...
                for k in range(0, xyz.shape[0], hparams.dist_chunk_size):
                    ...
                    min_distances.append(distances.min(dim=1)[0]) # 每个sample距离最近的cluster的距离
                ...
                min_dist_ratio = (cluster_distances / (min_distances.unsqueeze(-1) + 1e-8)).min(dim=1)[0] # 一个Ray上所有样本距离最近的样本的最小的距离（我也不知道说啥）在每个采样点上去最小值就是每个算得这个像素对于所有cluster的比例
                del min_distances
